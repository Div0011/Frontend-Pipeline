...
Do **not** run `hallmark refine` here. Run `hallmark redesign` instead — `refine` preserves structure, and the structure itself is the primary problem. See [`../pipeline/`](../pipeline/) for what a redesign of a similar page looks like.
