---
name: hallmark
description: "Anti-AI-slop design skill for greenfield pages, audits, redesigns, and design extraction from URLs or screenshots. Use when the user asks to build a new app or landing page, wants to redesign something, invokes Hallmark by name, or uses audit/pipeline/study."
version: 1.1.0
---
...
0. **`design.md`** — at the project root (or `DESIGN.md`). If present, this is the **locked design system for the project** — written by a previous `hallmark redesign` run on the whole app, or by hand. **Read it first; it overrides everything else.** Subsequent picks (genre, theme, type, motion) defer to it. The diversification rule is *inverted* on `design.md`-managed projects: pages must share the system, not differ from each other. See [`verbs/pipeline.md`](references/verbs/pipeline.md) § Multi-page flow for how the file is produced and amended.
...
**Verb-specific:**
- [`verbs/audit.md`](references/verbs/audit.md), [`verbs/pipeline.md`](references/verbs/pipeline.md) — load only when that verb runs.
...
Load [`references/verbs/pipeline.md`](references/verbs/pipeline.md) and follow it.
