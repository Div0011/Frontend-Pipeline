...
┌──────────────────────────────────────────────────────────────────────┐
│                         Client (web / CLI)                            │
│        POST /api/redesign  •  WS /api/pipeline/{id}/ws                │
└───────────────┬───────────────────────────────────┬──────────────────┘
                │ request                            │ live events
                ▼                                    │
┌───────────────────────────────────┐               │
...
Every agent has an editable Markdown prompt in `pipeline/prompts/`, loaded by
...
| `POST` | `/api/pipeline/{id}/resume` | resume a paused (HITL) run with a decision |
| `WS` | `/api/pipeline/{id}/ws` | live progress; client sends `resume` on interrupt |
...
# API:  uvicorn pipeline.api.main:app --reload   → http://localhost:8000/docs
```
