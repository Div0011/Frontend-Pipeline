...
def supervisor(state: RedesignState) -> Command:
    ...
    for t in ready:
        logger.info(
            "supervisor dispatching %s (seq: %s, responsibility: %s)",
            t.agent,
            t.operation_sequence or ["(none)"],
            t.responsibility or "(none)",
        )
...
def post_agent(state: RedesignState) -> Command:
    ...
    if last == "qa" and state.qa is not None and not state.qa.passed:
        if state.qa_failures < settings.max_qa_retries and state.qa.weakest_agent:
            ...
            # Build targeted rework payload from QA's structured output.
            qa = state.qa
            failing_checks: list[str] = []
            if qa.scores.genericness < 0.8:
                failing_checks.append(f"genericness ({qa.scores.genericness:.2f}): {', '.join(qa.genericness_triggers)}")
            if qa.scores.mobile_performance < 0.8:
                failing_checks.append(f"mobile_performance ({qa.scores.mobile_performance:.2f}): {', '.join(qa.mobile_triggers)}")
            if qa.scores.scroll_reversibility < 0.8:
                failing_checks.append(f"scroll_reversibility ({qa.scores.scroll_reversibility:.2f}): {qa.genre_sync_check}")
            for issue in qa.issues:
                failing_checks.append(f"{issue.agent}: {issue.title} — {issue.detail}")

            rework_detail = "\n".join(failing_checks) if failing_checks else qa.recommendation
            instructions = qa.rework_instructions or (
                f"QA failed on attempt {state.qa_failures + 1}.\n"
                f"Failing checks:\n{rework_detail}\n\n"
                f"Fix the specific defects above. Do not regenerate blindly."
            )
            ...
            return Command(
                update={
                    ...
                    "events": [emit("rework", "qa", f"Routing back to {target} with specific failure detail")],
                    "rework_payload": {
                        "target": target,
...
def synthesizer(state: RedesignState) -> dict:
    ...
    # Process media (extract WebP frame sequences if videos present)
    from pipeline.tools import process_project_media
    try:
        media_summary = process_project_media(str(store.root))
        if media_summary:
            logger.info("Processed project media frame sequences: %s", media_summary)
    except Exception as exc:
        logger.warning("Media processing skipped: %s", exc)

    # Automated build verification check (npx tsc --noEmit)
    build_log = ""
    try:
        import subprocess
        npm_bin = shutil.which("npm") or "npm"
        if (store.root / "package.json").exists():
            logger.info("Running automated build verification (tsc check) at %s", store.root)
            res = subprocess.run(
                [npm_bin, "run", "build"],
                cwd=str(store.root),
                capture_output=True,
        ...
        logger.warning("Automated build check failed to execute: %s", exc)
