...
* ``WS   /api/redesign/{id}/ws`` — live progress stream; supports resume when a
                                   human-approval interrupt fires.
* ``POST /api/redesign/{id}/resume`` — resume a paused run with a decision.
...
from redesign.runner import get_state_snapshot, resume_redesign, stream_redesign
