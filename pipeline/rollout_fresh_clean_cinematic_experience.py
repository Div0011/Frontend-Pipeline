#!/usr/bin/env python3
"""
Roll out Fresh, Clean, Cinematic Experience across all 24 projects:
- Refined glassmorphic containers with luminous micro-borders and soft ambient shadows
- Clean live status badges with pulsing brand indicator dots
- Polished, responsive interactive controls and modern typography
- Clean, uncluttered layout with generous breathing room
"""
...
def update_hero_fresh(project_dir: str, slug: str, cfg: dict):
    ...
    craft_type = "Wood-Fired Pizza" if food_type == "pizza" else "Craft Smash Atelier"
...
def update_smoothie_fresh(project_dir: str, slug: str, cfg: dict):
...
def update_manifesto_fresh(project_dir: str, slug: str, cfg: dict):
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        {{viewMode === "compare" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 font-sans">
            ...
            >
              <span className="font-sans text-xs font-bold uppercase tracking-wider block" style={{{{ color: "{primary}" }}}}>
                OUR CRAFT SPEC
              </span>
...
def main():
    print("🚀 Rolling out fresh, clean, cinematic enhancements across all 24 projects...")
    for slug, cfg in brand_configs.items():
        project_dir = os.path.join(PROJECTS_DIR, slug)
        if not os.path.exists(project_dir):
            continue

        update_hero_fresh(project_dir, slug, cfg)
        update_smoothie_fresh(project_dir, slug, cfg)
        update_manifesto_fresh(project_dir, slug, cfg)

        print(f"✓ Elevated {slug}")

    print("\n🎉 Fresh & clean cinematic rollout completed across all 24 projects!")
