...
logger = logging.getLogger("pipeline.tools.crawl")

_HEADING_TAGS = ("h1", "h2", "h3", "h4", "h5", "h6")
_HEX_RE = re.compile(r"#([0-9a-fA-F]{3}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})\b")
_FONT_RE = re.compile(r"font-family:\s*([^;]+)", re.IGNORECASE)
_SPACING_RE = re.compile(r"(?:margin|padding|gap|top|right|bottom|left):\s*([0-9]+(?:\.[0-9]+)?)(px|rem|em|vh|vw|%)")
...
class CrawlResult:
    ...
    detected_spacing: list[str] = field(default_factory=list)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "error": self.error,
            "detected_colors": self.detected_colors[:20],
            "detected_fonts": self.detected_fonts[:20],
            "detected_spacing": self.detected_spacing[:20],
        }


def _extract_design_tokens(html: str) -> tuple[list[str], list[str], list[str]]:
    """Extract colors, fonts, and spacing values from raw HTML/CSS."""
    colors: list[str] = []
    fonts: list[str] = []
    spacing: list[str] = []
    ...
    for m in _SPACING_RE.finditer(html):
        val = m.group(0).strip()
        if val not in spacing:
            spacing.append(val)

    return colors[:30], fonts[:20], spacing[:30]
...
def crawl_website(url: str, timeout: float = 20.0) -> CrawlResult:
    ...
    try:
        ...
        result.detected_colors, result.detected_fonts, result.detected_spacing = _extract_design_tokens(html)
