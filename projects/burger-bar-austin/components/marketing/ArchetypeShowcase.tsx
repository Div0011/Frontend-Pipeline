...
export default function ArchetypeShowcase() {
  ...
  const milestones = [
    { year: "1953", event: "Founding Flat-Top Seasoned", desc: "First burger served with the original family spice formula." },
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {milestones.map((m, idx) => (
            <div key={idx} className="p-6 rounded-xl bg-white/5 border border-white/10 space-y-3">
              <span className="type-display text-3xl font-black" style={{ color: "#2563EB" }}>
                {m.year}
              </span>
