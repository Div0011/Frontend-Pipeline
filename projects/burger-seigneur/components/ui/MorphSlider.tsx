      ...
      {showCaptions && (
        <div className="absolute inset-x-0 bottom-0 p-8 sm:p-12 z-20 flex flex-col justify-end">
          <div className="max-w-2xl space-y-3">
            {currentItem.tag && (
              <span className="font-mono text-xs uppercase tracking-widest px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-[#C8A96E] border border-white/20 inline-block">
                {currentItem.tag}
              </span>
            )}
            {currentItem.title && (
          ...
          {items.map((_, idx) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all duration-500 ${
                currentIndex === idx ? "w-8 bg-[#C8A96E]" : "w-2 bg-white/40 hover:bg-white/70"
              }`}
