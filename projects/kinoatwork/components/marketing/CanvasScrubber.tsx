...
/**
 * CanvasScrubber — scroll-driven frame sequence player for KINO ATWORK.
 *
 * Uses CSS sticky inner + GSAP ScrollTrigger watching outer wrapper progress.
 * No pin: true — eliminates pin-spacer jumps.
 *
 * Performance:
 *  - Frame 0 loads eagerly, canvas fades in on first-frame-ready.
 *  - Eager batch loaded on mount.
 *  - Remaining frames background-loaded at intervals.
...
export default function CanvasScrubber({
  ...
  return (
    ...
    >
      <div
        className="sticky top-0 h-[100svh] w-full overflow-hidden bg-cinema-bg"
        data-cursor="drag"
      >
        <canvas
          ref={canvasRef}
        ...
        {overlayGradient && (
          <>
            <div className="absolute inset-0 bg-gradient-to-t from-cinema-bg/75 via-cinema-bg/10 to-cinema-bg/15 pointer-events-none z-10" />
            <div className="absolute inset-0 bg-gradient-to-r from-cinema-bg/20 via-transparent to-transparent pointer-events-none z-10" />
          </>
        )}
