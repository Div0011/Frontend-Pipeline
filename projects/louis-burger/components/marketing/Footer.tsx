...
export default function Footer() {
  ...
  const socialLinks = [
    { name: "Instagram", handle: "@louis-burger", link: "https://instagram.com", icon: "📸" },
    { name: "Twitter / X", handle: "@LOUISBURGERHQ", link: "https://x.com", icon: "⚡" },
    { name: "TikTok", handle: "@louis-burger", link: "https://tiktok.com", icon: "🔥" },
    { name: "YouTube", handle: "Louis Burger Media", link: "https://youtube.com", icon: "🎬" },
  ];
        ...
        >
          {Array.from({ length: 6 }).map((_, i) => (
            <React.Fragment key={i}>
              <span className="flex items-center gap-2" style={{ color: "#D4AF37" }}>
                <span className="w-2 h-2 rounded-full animate-ping" style={{ backgroundColor: "#D4AF37" }} />
                LOUIS BURGER // 24K GOLD WAGYU & TRUFFLE MELT
              </span>
              <span className="text-stone-400">BENGALURU OUTPOSTS</span>
              <span className="text-white">CRAFT CULINARY ATELIER</span>
              <span style={{ color: "#D4AF37" }}>ORDER ONLINE · VIP TABLE BOOKING</span>
            </React.Fragment>
          ))}
        </motion.div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-12 md:px-20 py-20 space-y-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <p className="text-sm text-stone-300 max-w-md leading-relaxed font-body">
              CHEF ZORAWAR KALRA · CRAFT GOURMET BURGERS. Dedicated to artisanal craftsmanship, uncompromised ingredients, high-heat sears, and memorable culinary hospitality.
            </p>
          ...
          <div className="lg:col-span-6 p-8 rounded-3xl bg-white/[0.04] backdrop-blur-md border border-white/15 shadow-2xl space-y-6 relative overflow-hidden">
            <div className="space-y-2">
              <span className="font-mono text-[11px] font-bold uppercase tracking-widest" style={{ color: "#D4AF37" }}>
              ...
              {!isSubscribed ? (
                ...
                >
                  <div className="flex flex-col sm:flex-row gap-3">
                    <input
                      ...
                      className="flex-grow px-5 py-3.5 rounded-xl bg-black/60 border border-white/20 text-white placeholder-stone-400 font-mono text-xs focus:outline-none focus:border-[#D4AF37] transition-colors"
                    />
                    <button
                      type="submit"
                ...
                >
                  ...
                  <p className="font-mono text-xs text-stone-200">
                    Use code <span className="px-2 py-0.5 rounded bg-emerald-400/20 text-emerald-200 font-bold tracking-widest border border-emerald-400/40">LOUISB15</span> for 15% off your next visit.
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 pt-8 border-t border-white/10">
          ...
          <div className="space-y-3">
            <h4 className="font-mono text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2">
              ...
              Lavelle Road Atelier
            </h4>
            <p className="text-xs font-mono text-stone-300 leading-relaxed">
              Lavelle Rd, Shanthala Nagar
            </p>
            <div className="space-y-2 pt-1">
              <button
                type="button"
                onClick={() => copyToClipboard("+91 80 4567 8900")}
                className="text-xs font-mono text-stone-300 hover:text-white flex items-center gap-2 transition-colors"
              >
                <span>📞 +91 80 4567 8900</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/10" style={{ color: "#D4AF37" }}>
                  {copiedPhone === "+91 80 4567 8900" ? "COPIED!" : "COPY"}
                </span>
              </button>
              <a
                href="https://maps.google.com/?q=Louis+Burger+Bangalore"
                target="_blank"
            ...
            <ul className="space-y-2 text-xs font-mono text-stone-300">
              <li>
                <Link href="/menu" className="hover:text-white transition-colors flex items-center gap-1">
                  <span style={{ color: "#D4AF37" }}>→</span> <span>Signature Smashes</span>
                </Link>
              </li>
              <li>
                <Link href="/menu" className="hover:text-white transition-colors flex items-center gap-1">
                  <span style={{ color: "#D4AF37" }}>→</span> <span>Sides &amp; Thickshakes</span>
                </Link>
              </li>
              <li>
                <Link href="/reservations" className="hover:text-white transition-colors flex items-center gap-1">
                  <span style={{ color: "#D4AF37" }}>→</span> <span>VIP Table Booking</span>
                </Link>
              </li>
              <li>
                <Link href="/locations" className="hover:text-white transition-colors flex items-center gap-1">
                  <span style={{ color: "#D4AF37" }}>→</span> <span>Kitchen Outposts</span>
                </Link>
              </li>
            </ul>
          </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {socialLinks.map((s, idx) => (
                ...
                >
                  ...
                  <div className="mt-2">
                    <span className="text-[11px] font-mono font-bold text-white group-hover:text-white transition-colors block" style={{ color: "#D4AF37" }}>
                      {s.name}
                    </span>
        ...
        <div className="relative pt-12 border-t border-white/10 flex flex-col items-center justify-center overflow-hidden">
          ...
          >
            ...
            <span className="group-hover:-translate-y-1 transition-transform" style={{ color: "#D4AF37" }}>
              ↑
            </span>
          </button>
          ...
          <div className="w-full pt-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-[11px] font-mono text-stone-400">
            <p>© {new Date().getFullYear()} Louis Burger. All rights reserved.</p>
