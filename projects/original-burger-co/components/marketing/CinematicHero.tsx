      ...
      <div className="h-full max-w-[88rem] mx-auto px-6 lg:px-8">
        ...
        >
          <div className="space-y-6 max-w-3xl">
            <p className="type-caption text-[#1E3A8A] font-mono tracking-widest font-bold">
              Bangalore&apos;s Iconic Smashed Burger Diner · 5 Outlets
            </p>
            <h1 className="type-display text-5xl sm:text-7xl lg:text-8xl xl:text-[6.5rem] leading-[0.88] tracking-tight text-ink">
              The Original
              <br />
              <span className="text-[#1E3A8A]">Smash Standard</span>
            </h1>
            <p className="type-serif text-lg sm:text-xl md:text-2xl text-stone max-w-xl leading-relaxed">
              100% freshly pressed patties, caramelized cast-iron sear, melted sharp cheddar,
              and signature secret sauces across 5 iconic Bangalore neighborhoods.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <Link
                href="/menu"
                className="group inline-flex items-center gap-3 bg-[#1E3A8A] text-white px-8 py-4 type-caption text-xs hover:bg-[#F59E0B] hover:text-black transition-colors duration-500 shadow-xl"
              >
                Explore Smashed Menu
                <span className="group-hover:translate-x-1 transition-transform duration-300">→</span>
              </Link>
              <Link
                href="/locations"
                className="inline-flex items-center gap-3 border border-ink/30 text-ink px-8 py-4 type-caption text-xs hover:border-[#1E3A8A] hover:bg-[#1E3A8A] hover:text-white transition-all duration-500"
              >
                5 Bangalore Outlets
              </Link>
            </div>
        ...
        >
          <div className="space-y-6 max-w-2xl">
            <p className="type-caption text-[#1E3A8A] font-mono tracking-widest font-bold">
              Phase 01 / High Heat Smashed Sear
            </p>
            <h2 className="type-display text-4xl sm:text-6xl text-ink leading-tight">
              240°C Cast Iron<br />Lacquered Crust
            </h2>
            <p className="type-serif text-lg sm:text-xl text-stone leading-relaxed">
              Every fresh patty is smashed paper-thin onto raw high-heat iron within seconds,
              forming an intensely savory caramelized Maillard crust that locks inside all the juices.
            </p>
            <div className="flex items-center gap-4 text-xs font-mono text-stone">
              ...
              <span>OBC KINETICS SEQUENCE</span>
            </div>
          </div>
        </div>
        ...
        >
          <div className="space-y-6 max-w-xl text-left lg:text-right flex flex-col items-start lg:items-end w-full">
            <p className="type-caption text-[#1E3A8A] font-mono tracking-widest font-bold">
              Phase 02 / The OBC Double Stack
            </p>
            <h2 className="type-display text-4xl sm:text-6xl text-ink leading-tight">
              The OBC Classic<br />Double Smash
            </h2>
            <p className="type-serif text-lg sm:text-xl text-stone leading-relaxed">
              Two crispy seared smash patties, molten American cheese, diced sweet onions,
              house pickles, and signature OBC sauce nestled in warm buttered brioche.
            </p>
