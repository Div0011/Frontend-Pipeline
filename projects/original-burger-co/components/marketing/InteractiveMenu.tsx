        ...
        <div className="flex flex-col lg:flex-row items-stretch gap-0 border border-char/10">
                  ...
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      ...
                      <motion.h3
                        ...
                        className={`type-display text-3xl lg:text-4xl leading-[0.9] transition-colors duration-300 ${
                          isHovered ? "text-[#2563EB]" : "text-char"
                        }`}
                      >
                        {cat.label}
                      </motion.h3>
                    ...
                    <motion.span
                      ...
                      className={`text-lg transition-colors duration-300 ${
                        isHovered ? "text-[#2563EB]" : "text-smoke"
                      }`}
                    >
                      {isHovered ? "→" : "+"}
                    </motion.span>
                    ...
                    <div className="space-y-3">
                      {cat.items.slice(0, 2).map((item: MenuItem) => (
                        <div key={item.id} className="flex items-baseline justify-between">
                          <span className="type-body text-char text-sm group-hover:text-[#2563EB] transition-colors duration-300">
                            {item.name}
                          </span>
                      ...
                      {cat.items.length > 2 && (
                        <p className="type-label text-[#2563EB] text-[9px]">
          ...
          >
            <div className="text-center">
              <p className="type-caption text-[#2563EB] text-[9px] mb-3">Complete Selection</p>
              <motion.h3
                layout
              ...
              <motion.div
                layout
                className="inline-flex items-center gap-2 bg-ink text-[#2563EB] px-5 py-3 type-caption text-[10px]"
              >
                VIEW ALL →
              </motion.div>
            </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-50 bg-[#2563EB] flex flex-col"
            role="dialog"
            aria-modal="true"
            aria-label="Full menu"
          >
            <div className="flex items-center justify-between px-6 lg:px-8 h-20 border-b border-char/10 flex-shrink-0">
              <span className="type-display text-2xl text-char">ORIGINAL BURGER CO.</span>
              <button
              ...
              <div className="max-w-[88rem] mx-auto px-6 lg:px-8 py-12 lg:py-20">
                ...
                <div className="mt-20 pt-10 border-t border-char/15 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                  ...
                  <Link
                    href="/reservations"
                    onClick={() => setMenuOpen(false)}
                    className="inline-flex items-center gap-3 bg-char text-[#2563EB] px-8 py-4 type-caption text-[11px] hover:bg-char-soft transition-colors duration-300"
                  >
                    Book a Table →
                  </Link>
                </div>
